...
def register():
    ...
    if request.method == 'POST' and 'first_name' in request.form and 'last_name' in request.form and 'email_add' in request.form and 'password' in request.form and 'prn' in request.form:
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute(
            'SELECT * FROM accounts WHERE email_add = %s', (email_add,))
        account = cursor.fetchone()
        if account:
        ...
        try:
            if 'file' in request.files:
                ...
                if file.filename != '':
                    ...
                    cur.execute("INSERT INTO books(dept_id,dept, bk_id,author,bk_name, bk_des, file_path, Quantity) VALUES (%s,%s,%s, %s, %s, %s,%s, %s)", (
                        dept_id, dept, bk_id, author, bk_name, bk_des, file_path, Quantity))
...
def edit_in_chem(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'Chemical' AND sr_no = %s", (sr_no,))
    ...
    if request.method == "POST":
        ...
        filenaemEdit = secure_filename(
            bk_file.filename)  # securing name of file
        # joining folder path with filename
        filepathEdit = os.path.join(app.config['UPLOAD_FOLDER'], filenaemEdit)
...
def edit_in_comp(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'Computer' AND sr_no = %s", (sr_no,))
    book_data = cursor.fetchone()
    old_filename = book_data['file_path']  # retrieving old img path from db
    if request.method == "POST":
        bk_name = request.form["bk_name"]
...
def edit_in_it(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'InformationTech' AND sr_no = %s", (sr_no,))
    book_data = cursor.fetchone()
    old_filename = book_data['file_path']  # retrieving old img path from db
    if request.method == "POST":
        bk_name = request.form["bk_name"]
...
def edit_in_instru(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'Instrumentation' AND sr_no = %s", (sr_no,))
    book_data = cursor.fetchone()
    old_filename = book_data['file_path']  # retrieving old img path from db
    if request.method == "POST":
        bk_name = request.form["bk_name"]
...
def edit_in_mech(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'Mechinacal' AND sr_no = %s", (sr_no,))
    book_data = cursor.fetchone()
    old_filename = book_data['file_path']  # retrieving old img path from db
    if request.method == "POST":
        bk_name = request.form["bk_name"]
...
def edit_in_entc(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'ENTC' AND sr_no = %s", (sr_no,))
    book_data = cursor.fetchone()
    old_filename = book_data['file_path']  # retrieving old img path from db
    if request.method == "POST":
        bk_name = request.form["bk_name"]
...
def edit_in_aids(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'AIDS' AND sr_no = %s", (sr_no,))
    book_data = cursor.fetchone()
    old_filename = book_data['file_path']  # retrieving old img path from db
    if request.method == "POST":
        bk_name = request.form["bk_name"]
...
def edit_in_csai(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'CSAI' AND sr_no = %s", (sr_no,))
    book_data = cursor.fetchone()
    old_filename = book_data['file_path']  # retrieving old img path from db
    if request.method == "POST":
        bk_name = request.form["bk_name"]
...
def edit_in_csaiml(sr_no):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute(
        "SELECT * FROM books WHERE dept = 'CSAIML' AND sr_no = %s", (sr_no,))
    book_data = cursor.fetchone()
    old_filename = book_data['file_path']  # retrieving old img path from db
    if request.method == "POST":
        bk_name = request.form["bk_name"]
...
def search():
    ...
    cursor.execute(
        "create or replace view search as select bk_name ,bk_id ,dept , author from books where author = %s", (auth,))
    ...
    if request.method == "POST":
        ...
        # datetime_return = date_time_return.strftime("%B %d, %Y")
            ...
            try:
                ...
                while scanning:
                    # This line reads a frame from the camera feed. ret is a boolean indicating whether the frame was successfully read, and frame contains the image data.
                    ret, frame = cap.read()
                    if not ret:
                        continue
                    ...
                    # Press 'Esc' to exit the barcode scanner.
                ...
                cur = mysql.connection.cursor()
                cur.execute("INSERT INTO book_issue (book1_name, date_time, prn, count,date_time_return , dept_id) VALUES (%s, %s, %s, %s, %s, %s)",
...
def return_book():
    ...
    return render_template('return_books.html', data=data)


@app.route('/return_confirm', methods=['GET', 'POST'])
def return_confirm():
    ...
    if confirm == "yes" or "no":
        try:
            cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
            cur.execute(
                'DELETE FROM book_issue WHERE book1_name = %s AND prn = %s', (book, prn))
            mysql.connection.commit()
            print("done")
            cur.close()
        except Exception as e:
...
def all_books():
    ...
    return render_template('all_books.html', data=data)
